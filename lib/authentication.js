...
module.exports = function(opts) {
    ...
    opts.app.consumer = new oauth.OAuth(
      ...
      opts.app.get('/sessions/connect', function(req, res) {
          opts.app.consumer.getOAuthRequestToken(function(error, oauthToken, oauthTokenSecret, results) {
              if (error) {
                  ...
                  console.log('error', error, _twitterConsumerKey, _twitterConsumerSecret, callbackURL);
              } else {
      ...
      opts.app.get('/sessions/callback', function(req, res) {
          opts.app.consumer.getOAuthRequestToken(req.session.oauthRequestToken, req.session.oauthRequestTokenSecret, req.query.oauth_verifier, function(error, oauthAccessToken, oauthAccessTokenSecret, results) {
              if (error) {
